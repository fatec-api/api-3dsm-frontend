...
/* 
// INTEGRAÇÃO REAL (DESCOMENTAR QUANDO O BACKEND ESTIVER ONLINE)
try {
    // Realiza a chamada PUT para a URL definida no arquivo de configuração da API
    const response = await api.put(`apontamentos/${id}`, dados);
    return response.data;
} catch (error) {
    // Tratamento de erro estruturado para observabilidade
    console.error(`[API_ERROR] Falha na atualização do apontamento ${id}:`, error);
    throw error;
}
*/
...
export const getApontamentosAprovadosPorProjeto = async (_projetoId: number): Promise<MetricasAtividade[]> => {
