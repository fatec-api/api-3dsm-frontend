...
export async function atualizarApontamento(id: string, dados: any) {
    try {
        console.info(`[API] Solicitando PUT em /apontamentos/${id}`, dados);
        const response = await api.put(`apontamentos/${id}`, dados);
        return response.data;
    } catch (error) {
        console.error(`[ERROR] Falha na atualização do apontamento ${id}:`, error);
        throw error;
    }
}
...
export async function reprovarApontamento(id: string, justificativa: string) {
    console.info(`[ACTION] Solicitando reprovação ID: ${id} | Motivo: ${justificativa}`);
    return new Promise((resolve) => {
        setTimeout(() => {
            resolve({ message: "Reprovado com sucesso", id, justificativa });
        }, 800);
    });
}