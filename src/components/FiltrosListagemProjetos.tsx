...
export default function FiltrosListagemProjetos({ projetos, onFilterChange }: FiltrosListagemProjetosProps) {
    ...
    const getProgresso = (projeto: Projeto): string => {
        ...
        const percentualConsumo = (projeto.horasRealizadasTotal / projeto.horasPrevistasTotal) * 100;
    ...
    const progressos = useMemo(() => {
        const projetosComProgresso = projetos.map(p => ({ ...p, progresso: getProgresso(p) }));
    ...
    return (
        <div className="flex flex-col md:flex-row items-center justify-center p-5 mb-4 gap-4">
            ...
            <select className="select select-bordered w-40" value={progresso} onChange={(e) => setProgresso(e.target.value)}>
            ...
            <select className="select select-bordered w-40" value={statusProjeto} onChange={(e) => setStatusProjeto(e.target.value)}>
            ...
            <select className="select select-bordered w-40" value={tipoProjeto} onChange={(e) => setTipoProjeto(e.target.value)}>
                <option value="">Tipo</option>
                <option value="Alocacao">Alocação</option>
                <option value="Hora_Fechada">Hora Fechada</option>
            </select>

            <select className="select select-bordered w-40" value={cliente} onChange={(e) => setCliente(e.target.value)}>
